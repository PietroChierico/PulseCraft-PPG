...
/*
  ESP32-C6 Feather + MAX30102 / MAX30105 PPG WiFi Red and Infrared Streamer
...
const char* AP_SSID     = "PPG_STREAM_group4";
...
const byte LED_MODE = 2;           // 1 = Red only, 2 = Red + Infrared, 3 = Red + Infrared + Green
...
// Initialize onboard NeoPixel pin.
void setupNeoPixel() {
#if defined(PIN_NEOPIXEL)
...
void setupSensor() {
  ...
  Serial.println("Stream format: red,infrared");
}
...
void acceptClientIfNeeded() {
  ...
  if (newClient) {
    ...
    // Send a CSV header to the PC when it connects.
    client.println("red,infrared");

    setNeoColor(0, 32, 32);   // Cyan: PC connected to TCP stream

    Serial.println();
    Serial.println("PC connected to PPG Red and Infrared stream.");
  }
}

void streamSample(uint32_t red, uint32_t infrared) {
  char line[48];
...
void setup() {
  ...
  Serial.println();
  Serial.println("ESP32-C6 Feather PPG WiFi Red and Infrared Streamer");
...
void loop() {
  ...
  while (ppg.available()) {
    ...
    uint32_t infrared = ppg.getFIFOIR();

    streamSample(red, infrared);
