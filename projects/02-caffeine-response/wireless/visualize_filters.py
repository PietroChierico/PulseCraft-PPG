...
def read_wifi_values(wifi_client):
    """
    Read numeric IR values from the WiFi TCP stream.
...
def setup_plot():
    ...
    ax.set_ylabel("IR PPG value")
    ax.set_title("Real-Time WiFi IR PPG Signal")
    ax.grid(True)
    ax.legend()
...
def main():
    ...
    try:
        ...
        while running:
            values = read_wifi_values(wifi_client)
