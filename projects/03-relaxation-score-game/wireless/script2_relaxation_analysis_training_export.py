...
Purpose
-------
Load the IR CSV exported from Script 1, compare every PPG feature between
...
def add_legacy_feature_compatibility(data: pd.DataFrame) -> pd.DataFrame:
    """
    Let this IR version load older CSV files that used red feature names.
    New exports use mean_ir/std_ir/range_ir/rms_ir/mad_ir.
    """
    data = data.copy()
...
class RelaxationTrainingWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.subtitle_label = QLabel("Generate one boxplot per IR feature: Pre-Relaxation vs Post-Relaxation, then export the trained model.")
    ...
    def load_data(self) -> None:
        ...
        if not self.available_features:
            QMessageBox.critical(self, "Invalid dataset", "No expected IR feature columns were found.")
            return
    ...
    def export_model(self) -> None:
        if self.model_bundle is None:
            ...
            return
        default_name = f"ppg_relaxation_ir_model_{int(time.time())}.joblib"
