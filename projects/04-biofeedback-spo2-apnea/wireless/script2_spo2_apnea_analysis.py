...
apnea protocol. When Red and Infrared raw columns are present, this script
recalculates SpO2 from the Red/Infrared ratio of ratios before analysis.
The app compares the SpO2 trend across protocol phases:
    Baseline vs Apnea vs Recovery

Main tools
----------
1. Box plot: SpO2 distribution by phase.
2. Time plot: SpO2 over time, colored by phase, for the selected subject/trial.
3. Histogram explorer: SpO2 histograms by phase.
4. Subject/trial summary table with apnea duration, oxygen drop, and recovery level.
...
from ppg_wifi_ir_stream import estimate_spo2_from_red_ir_windows
...
def recalculate_spo2_from_red_ir(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recalculate SpO2 from raw Red and Infrared samples when those columns exist.

    This keeps Script 2 aligned with Script 1 even if an older CSV contains stale
    or placeholder SpO2 values. If the CSV does not contain enough valid Red/IR
    samples, the existing SpO2 column is kept unchanged.
    """
    if not {"red_raw", "ir_raw"}.issubset(df.columns):
        return df

    clean = df.copy()
    clean["red_raw"] = pd.to_numeric(clean["red_raw"], errors="coerce")
    clean["ir_raw"] = pd.to_numeric(clean["ir_raw"], errors="coerce")
    if "ratio_of_ratios" in clean.columns:
        clean["ratio_of_ratios"] = pd.to_numeric(clean["ratio_of_ratios"], errors="coerce")

    window_samples = int(SPO2_WINDOW_SECONDS * SAMPLING_RATE_HZ)
    valid_pairs = clean[["red_raw", "ir_raw"]].dropna()
    if len(valid_pairs) < window_samples:
        return clean

    for _, index_values in clean.groupby(["subject_id", "trial_id"]).groups.items():
        trial = clean.loc[list(index_values)].sort_values("elapsed_trial_s")
        if len(trial[["red_raw", "ir_raw"]].dropna()) < window_samples:
            continue
        ...
        for row_index, row in trial.iterrows():
            red_raw = row["red_raw"]
            ir_raw = row["ir_raw"]
            if np.isfinite(red_raw) and np.isfinite(ir_raw):
                red_values.append(float(red_raw))
                ir_values.append(float(ir_raw))
                if len(red_values) > window_samples:
                    red_values = red_values[-window_samples:]
                    ir_values = ir_values[-window_samples:]

            elapsed = float(row["elapsed_trial_s"])
            if elapsed - last_update_time >= SPO2_UPDATE_SECONDS and len(red_values) >= window_samples:
                current_spo2, current_ratio = estimate_spo2_from_red_ir_windows(
                    np.asarray(red_values[-window_samples:], dtype=float),
                    np.asarray(ir_values[-window_samples:], dtype=float),
            ...
            clean.at[row_index, "spo2_estimate"] = current_spo2
            if "ratio_of_ratios" in clean.columns:
                clean.at[row_index, "ratio_of_ratios"] = current_ratio
...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    clean = clean.dropna(subset=["trial_id", "elapsed_trial_s"])
    clean["trial_id"] = clean["trial_id"].astype(int)
    clean = clean[clean["phase"].isin(PHASE_ORDER)]
    clean = clean.sort_values(["subject_id", "trial_id", "elapsed_trial_s"]).reset_index(drop=True)
    clean = recalculate_spo2_from_red_ir(clean)
    clean = clean.dropna(subset=["spo2_estimate"])
...
class SpO2AnalysisWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel(
            "Explore how the Red/IR SpO2 estimate changes across Baseline, Voluntary Apnea, and Recovery. "
            "Educational signal-analysis tool only, not a medical diagnostic device."
        )
        subtitle.setStyleSheet("font-size: 15px; color: #c9d2e3;")
        self.status = QLabel("Load a WiFi Red + IR CSV exported from Script 1.")
