...
When Red and Infrared raw columns are present, this script recalculates SpO2
from the Red/Infrared ratio of ratios before scoring.

Game idea
---------
A high score is achieved by:
    1. Longer voluntary apnea duration.
...
def recalculate_spo2_from_red_ir(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This keeps the leaderboard scoring aligned with the updated collection
    script. If the CSV does not contain enough valid Red/IR samples, the existing
    SpO2 column is kept unchanged.
    """
...
class SpO2LeaderboardGameWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel(
            "Challenge: keep the oxygenation estimate high while maintaining a longer voluntary apnea. "
