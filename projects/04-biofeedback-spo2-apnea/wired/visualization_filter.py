...
"""
PPG Serial Reader - Real-Time Infrared Version with GUI Buttons

Expected serial input:
Each serial line must contain two numeric values:
...
The program ignores the first number, Red, and uses only the second number, Infrared.
...
# Regex used to extract numeric values from each serial line
NUMBER_PATTERN = re.compile(r"[-+]?(?:\d*\.\d+|\d+)(?:[eE][-+]?\d+)?")


def read_infrared_value(serial_connection):
    """
    Read one line from the serial port and extract the infrared value.

    The serial line is expected to contain at least two numbers:
    - first number: Red
    - second number: Infrared

    Only the second number is returned.
    Returns None if the received data is not valid.
    """
    ...
    try:
        infrared_value = float(numbers[1])
...
def setup_plot():
    ...
    ax.set_title("Real-Time Infrared PPG Signal")
    ax.grid(True)
    ax.legend()

    # Stop the program if the user closes the plot window
...
def print_startup_info():
    ...
    """
    print("=== PPG Real-Time Infrared Reader ===")
    ...
    print("")
    print("Serial input format:")
    print("Each line must contain two numbers: Red and Infrared")
    print("Only the second number, Infrared, will be used")
    print("")
...
def main():
    ...
    try:
        ...
        while running:
            infrared_value = read_infrared_value(serial_connection)
